...
- 自分の**ホームディレクトリ**は **~**(チルダ)
- ユーザーid nagisa　の**ホームディレクトリ**は **~nagisa**(チルダ+ユーザーid)

例えば図2.1で**ホームディレクトリ** harukoにいたとすると：
- **親ディレクトリ**Usersは **\.\.**(ドット2つ)
- **親ディレクトリ**Usersは **~/\.\.**(チルダ+/+ドット2つ)
- ルートディレクトリは2階層上なので **\.\./\.\.**
- ディレクトリ /Users/nagisaは **~nagisa**(チルダ+ユーザーid)
- ディレクトリ /Usersは **~nagisa/\.\.**(チルダ+ユーザーid+/+ドット2つ)
...
絶対パスで指定します。「haruko」の部分は自分のユーザーidに変更してください。
ターミナルに以下のコマンドを入力し、実行してください。
```sh
cd /User/haruko/dojodir
```
パスは他には
`haruko/dojodir`(相対パス、「haruko」の部分は自分のユーザーidに変更してください)、`~/dojodir` (相対パス)で入力することができます。
パスを確認してみましょう。
ターミナルに以下のコマンドを入力し、実行してください。
```sh
pwd
```
出力例: 「haruko」の部分は自分のユーザーidとなります。
```sh
~:$ pwd
/Users/haruko/dojodir
```
